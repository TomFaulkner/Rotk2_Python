...
class Command4(object):
    ...
    def Start(self, province_no):
        ...
        commands = [
            Helper.GetBuiltinText(0x6C66, 0x6C69),
            Helper.GetBuiltinText(0x6C6E, 0x6C71),
            Helper.GetBuiltinText(0x6C76, 0x6C79),
        ]
        Helper.ClearInputArea()
        Helper.ShowCommandsInInputArea(commands, 3, width=60, palette_no=5)
        cmd = Helper.GetInput(
            Helper.GetBuiltinText(0x7E5C, 0x7E61) + "(1-3)? ",
            row=1,
    ...
    def MuBing(self):
        ...
        officer_no = Helper.SelectOfficer(
            self.province_no, Helper.GetBuiltinText(0x6C38), ShowOfficerFlag.Empty
        )
        # how many soldiers will be hired?
        soldiers_hired = Helper.GetInput(
            Helper.GetBuiltinText(0x6C41) + "(1-{0})? ".format(capacity),
            row=1,
            required_number_min=1,
            required_number_max=capacity,
        )
        if soldiers_hired < 0:
            return
    ...
    def PaiBing(self):
        # who will hire soldiers
        officer_no = Helper.SelectOfficer(
            self.province_no, Helper.GetBuiltinText(0x6C2B), ShowOfficerFlag.Empty
        )
    ...
    def Training(self):
        ...
        officer_no = Helper.SelectOfficer(
            self.province_no, Helper.GetBuiltinText(0x6BA0), ShowOfficerFlag.War
        )
        if officer_no < 1:
            return
        ...
        Helper.ShowDelayedText(
            Helper.GetBuiltinText(0x6BB9), palette_no=3, top=330, clear_input_area=False
        )
    ...
    def AdjustSoldiers(self, soldiers_hired):
        ...
        while True:
            ...
            img = Helper.DrawText(
                Helper.GetBuiltinText(0x6BF2, 0x6BFF).replace("%lu", str(soldiers_remained * 100)),
                scaled=True,
                palette_no=5,
            )
            ...
            officer_no = Helper.SelectOfficer(
                self.province_no,
                Helper.GetBuiltinText(0x6C06),
                ShowOfficerFlag.Soldiers,
                row=1,
                clear_area=False,
            ...
            if officer_no < 1:
                if soldiers_remained > 0:
                    ...
                    yn = Helper.GetInput(
                        Helper.GetBuiltinText(0x6C16).replace("%lu", str(soldiers_remained * 100))
                        + Helper.GetBuiltinText(0x3D7D)
                        + "(Y/N)? ",
                        yesno=True,
                        row=2,
                    )
                    ...
                    yn = Helper.GetInput(
                        Helper.GetBuiltinText(0x3D7D) + "(Y/N)? ", yesno=True, row=2
                    )
                    if yn == "y":
            ...
            soldiers = Helper.GetInput(
                Helper.GetBuiltinText(0x6BE2) + "(0-{0})".format(max * 100),
                row=2,
                required_number_min=0,
                required_number_max=max * 100,
            )
            if soldiers >= 0:
