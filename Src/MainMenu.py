...
class MainMenu(object):
    def __init__(self):
        self.mm = None
        self.scenario_name = [
            Helper.GetBuiltinText(0x4F9E),
            Helper.GetBuiltinText(0x4FAC),
            Helper.GetBuiltinText(0x4FBA),
            Helper.GetBuiltinText(0x4FC8),
            Helper.GetBuiltinText(0x4FD6),
            Helper.GetBuiltinText(0x4FE4),
        ]
    ...
    def Start(self):
        ...
        while True:
            ...
            cmd = Helper.GetInput(
                Helper.GetBuiltinText(0x5342) + "(1-3)? ",
                250,
                250,
                width=200,
    ...
    def EndGame(self):
        yn = Helper.GetInput(
            Helper.GetBuiltinText(0x534D) + "(Y/N)? ", x=250, y=300, width=200, yesno=True
        )
        if yn == "y":
            return "OK"
    ...
    def SelectScenario(self):
        ...
        pygame.draw.rect(
            Helper.Screen,
            (0, 0, 0),
            (250 * Helper.Scale, 50 * Helper.Scale, 230 * Helper.Scale, 300 * Helper.Scale),
        )
        pygame.display.flip()
        ...
        for i in range(0, 6):
            ...
            img = Helper.DrawText(
                "{0}{1}".format(year, Helper.GetBuiltinText(0x3DD2, 0x3DD3)),
                scaled=True,
                palette_no=1,
            )
        ...
        no = Helper.GetInput(
            Helper.GetBuiltinText(0x52FC) + "(1-6)? ",
            x=285,
            width=200,
            required_number_min=1,
    ...
    def ShowScenario(self, bmp):
        ...
        bmp.blit(img, (5, 10))
        img = Helper.DrawText(
            "{0}.{1}".format(self.scenario_no + 1, self.scenario_name[self.scenario_no])
        )
        bmp.blit(img, (100, 10))
    ...
    def SelectPlayers(self, bmp):
        ...
        img = Helper.DrawText(
            Helper.GetBuiltinText(0x5559), scaled=True, palette_no=0, back_color=(255, 255, 255)
        )
        Helper.Screen.blit(img, (32 * Helper.Scale, 10 * Helper.Scale))

        max_rulers = min(limit[self.scenario_no], len(ruler_list) + 1)
        num = Helper.GetInput(
            Helper.GetBuiltinText(0x5290) + "(0-{0})? ".format(max_rulers),
            required_number_min=0,
            required_number_max=max_rulers,
            allow_enter_exit=False,
        )
        ...
        for i in range(0, num):
            ...
            while True:
                ...
                if limit[self.scenario_no] > 6:
                    ruler_no = Helper.GetInput(
                        "{2}{0},{3}(0-{1})? ".format(
                            i + 1,
                            max_rulers,
                            Helper.GetBuiltinText(0x5299, 0x529E),
                            Helper.GetBuiltinText(0x52A3, 0x52A8),
                        ),
                        next_prompt=Helper.GetBuiltinText(0x52B3),
                        required_number_min=0,
                        required_number_max=max_rulers,
                        allow_enter_exit=False,
                    )
    ...
    def ShowLevel(self, bmp):
        ...
        level_mappings = {
            1: Helper.GetBuiltinText(0x5530),
            2: Helper.GetBuiltinText(0x5535),
            3: Helper.GetBuiltinText(0x553A),
        }
    ...
    def SelectLevel(self, bmp):
        ...
        self.level = Helper.GetInput(
            "1."
            + Helper.GetBuiltinText(0x5530)
            + " 2."
            + Helper.GetBuiltinText(0x5535)
            + " 3."
            + Helper.GetBuiltinText(0x553A),
            next_prompt=Helper.GetBuiltinText(0x55C0) + "(1-3)? ",
    ...
    def SeeWar(self, bmp):
        Helper.ClearInputArea()
        see_war = Helper.GetInput(
            Helper.GetBuiltinText(0x55E8) + "(Y/N)? ", allow_enter_exit=False, keydown_mode=True
        )  # 嚒
    ...
    def SelectHistory(self, bmp):
        ...
        self.history = Helper.GetInput(
            "1." + Helper.GetBuiltinText(0x553F) + " 2." + Helper.GetBuiltinText(0x5548),
            next_prompt=Helper.GetBuiltinText(0x561F) + "(1-2)? ",
            cursor_user_prompt_location=True,
            required_number_min=1,
    ...
    def NewGame(self):
        ...
        while True:
            Helper.ClearInputArea()
            yn = Helper.GetInput(
                Helper.GetBuiltinText(0x558C) + Helper.GetBuiltinText(0x3D7D) + "(Y/N)? ",
                yesno=True,
            )
            if yn in ["", "n"]:
                cmd = Helper.GetInput(
                    Helper.GetBuiltinText(0x558C) + "(0-5)? ",
                    required_number_min=0,
                    required_number_max=5,
                )
                if cmd < 1:
                    return -1
                if cmd == 1:
    ...
    def NewRuler_GetName(self):
        Helper.ClearInputArea()
        eng_chn = Helper.GetInput(
            Helper.GetBuiltinText(0x57BE, 0x57BF)
            + Helper.GetBuiltinText(0x57B9)
            + Helper.GetBuiltinText(0x57C2),
            next_prompt=Helper.GetBuiltinText(0x57C6)
            + Helper.GetBuiltinText(0x57DC)
            + "    "
            + "(1-2)? ",
            cursor_user_prompt_location=True,
            required_number_min=1,
        ...
        if eng_chn == 1:
            while True:
                ...
                name = Helper.GetInput(
                    "", 350, 345, width=200, palette_no=1, required_number=False, max_chars=6
                )
                yn = Helper.GetInput(
                    Helper.GetBuiltinText(0x57F9) + Helper.GetBuiltinText(0x3D7D) + "(Y/N)? ",
                    keydown_mode=True,
                )
                if yn in ["", "n"]:
                    continue
                ...
                pygame.draw.rect(
                    Helper.Screen,
                    (0, 0, 0),
                    (400 * Helper.Scale, 40 * Helper.Scale, 100 * Helper.Scale, 30 * Helper.Scale),
                )
    ...
    def NewRuler_GetBirthday(self):
        Helper.ClearInputArea()
        birth_month = Helper.GetInput(
            Helper.GetBuiltinText(0x5387) + "(1-12)? ",
            required_number_min=1,
            required_number_max=12,
            allow_enter_exit=False,
        )
        birth_day = Helper.GetInput(
            Helper.GetBuiltinText(0x53B0) + "(1-31)? ",
            required_number_min=1,
            required_number_max=31,
            allow_enter_exit=False,
        )
        Helper.ClearInputArea()
        age = Helper.GetInput(
            Helper.GetBuiltinText(0x53BD) + "(1-99)? ",
            required_number_min=1,
        ...
        img = Helper.DrawText(
            "{0}{3} {1}{4}    {2}{5}".format(
                ...
                birth_day,
                age,
                Helper.GetBuiltinText(0x5849),
                Helper.GetBuiltinText(0x584E),
                Helper.GetBuiltinText(0x5854),
            ),
            scaled=True,
        )
        Helper.Screen.blit(img, (400 * Helper.Scale, 75 * Helper.Scale))

        Data.BUF[Data.RULER_AS_OFFICER_OFFSET + 0x19] = (
            Data.BUF[0x45] * 256 + Data.BUF[0x44] + 2 - age
        )
        return age
    ...
    def NewRuler_GetSex(self, age):
        Helper.ClearInputArea()
        sex = Helper.GetInput(
            "1." + Helper.GetBuiltinText(0x536E) + " 2." + Helper.GetBuiltinText(0x5373),
        ...
        img = Helper.DrawText(
            [Helper.GetBuiltinText(0x536E), Helper.GetBuiltinText(0x53DB)][sex - 1], scaled=True
        )
    ...
    def NewRuler_GetAbility(self, sex, age_index):
        ...
        while True:
            ...
            cmd = Helper.GetInput(
                determin,
                next_prompt=Helper.GetBuiltinText(0x5409),
                cursor_user_prompt_location=True,
            ...
            if award == 0:
                Helper.ClearInputArea()
                yn = Helper.GetInput(
                    Helper.GetBuiltinText(0x5464) + Helper.GetBuiltinText(0x3D7D) + "(Y/N)? ",
                    yesno=True,
                    keydown_mode=True,
                )  # 麽麼
                if yn == "y":
        ...
        face = pygame.transform.scale(
            face, (face.get_width() * Helper.Scale, face.get_height() * Helper.Scale)
        )
    ...
    def NewRuler_GetRuledProvince(self):
        while True:
            Helper.ClearInputArea()
            pno = Helper.GetInput(
                Helper.GetBuiltinText(0x54A8) + "(1-41)? ",
                required_number_min=1,
    ...
    def NewRuler_AddFollower(self):
        name = ""
        yn = Helper.GetInput(
            Helper.GetBuiltinText(0x548F) + "(Y/N)? ", yesno=True, keydown_mode=True
        )
        if yn == "y":
            Helper.ClearInputArea()
            eng_chn = Helper.GetInput(
                Helper.GetBuiltinText(0x57BE, 0x57BF)
                + Helper.GetBuiltinText(0x57B4)
                + Helper.GetBuiltinText(0x57C2),
                next_prompt=Helper.GetBuiltinText(0x57C6)
                + Helper.GetBuiltinText(0x57DC)
                + "    "
            ...
            if eng_chn == 1:
                while True:
                    ...
                    name = Helper.GetInput(
                        "", 350, 335, width=200, palette_no=1, required_number=False, max_chars=6
                    )
                    yn = Helper.GetInput(
                        Helper.GetBuiltinText(0x57F9) + Helper.GetBuiltinText(0x3D7D) + "(Y/N)? ",
                        keydown_mode=True,
                ...
                img = Helper.DrawText(
                    "{0}{1}".format(age, Helper.GetBuiltinText(0x5854)), scaled=True
                )
                ...
                img = Helper.DrawText(
                    [Helper.GetBuiltinText(0x536E), Helper.GetBuiltinText(0x53DB)][sex - 1],
                    scaled=True,
                )
        ...
        follower = Officer.FromBuffer(
            Data.BUF[follower_offset : follower_offset + 0x2B].copy(),
            253,
            Data.GetWordFromOffset(Data.BUF, 0x44),
        )
    ...
    def NewRuler(self):
        ...
        while True:
            ...
            if yn in ["", "n"]:
                change_no = Helper.GetInput(
                    Helper.GetBuiltinText(0x54BE) + "(1-6)? ",
                    required_number_min=1,
                    required_number_max=6,
                    allow_enter_exit=True,
                )
    ...
    def show_values(self, index, values, award):
        cols = [
            Helper.GetBuiltinText(0x5378),
            Helper.GetBuiltinText(0x537D),
            Helper.GetBuiltinText(0x5382),
        ]

        pygame.draw.rect(
            Helper.Screen,
            (0, 0, 0),
            (400 * Helper.Scale, 140 * Helper.Scale, 200 * Helper.Scale, 70 * Helper.Scale),
        )
    ...
    def LoadGame(self):
        while True:
            pygame.draw.rect(
                Helper.Screen,
                (0, 0, 0),
                (250 * Helper.Scale, 100 * Helper.Scale, 200 * Helper.Scale, 200 * Helper.Scale),
            )
            pygame.display.flip()
    ...
    def NewGameSelectRuler(self, no, page, player_list):
        ...
        for i in range(6 * page, max_rulers):
            ...
            if i in player_list:
                palette_no = 4
            name_bmp = Helper.DrawText(
                "{0}".format(name_indexes), scaled=False, palette_no=palette_no
            )
            bmp.blit(
                name_bmp, (left + 20 + 110 * int((i % 3)), top + 5 + height + 140 * int(i % 6 / 3))
            )
