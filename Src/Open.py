...
class Open(object):
    ...
    def GetSplashScreen(self, keyname) -> pygame.Surface:
        ...
        # Use palette 0 or 1 if 7 is out of range (defensive)
        palette_no = 7
        if len(Helper.Palettes) <= palette_no:
            palette_no = min(1, len(Helper.Palettes) - 1) if Helper.Palettes else 0

        img = Helper.DrawData(dc.display_buf, palette_no)
    ...
    def ShowSplashScreen(self):
        ...
        if splash is not None:
            if self.splash_index == 1:
                ...
                splash = splash.subsurface(
                    pygame.Rect(0, 250 * Helper.Scale, 640 * Helper.Scale, 100 * Helper.Scale)
                )
