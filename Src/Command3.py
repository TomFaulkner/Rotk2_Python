...
class Command3(object):
    ...
    def Start(self, province_no):
        ...
        for r in range(0, 12):
            for c in range(0, 13):
                ...
                bmp_mini_map = pygame.transform.scale(
                    bmp_mini_map,
                    (
                        bmp_mini_map.get_width() * Helper.Scale,
                        bmp_mini_map.get_height() * Helper.Scale,
                    ),
                )
                bmp.blit(
                    bmp_mini_map,
                    (c * 32 * Helper.Scale, r * 32 * Helper.Scale + (c % 2) * 16 * Helper.Scale),
                )
        ...
        bmp_warmenu_sub = bmp_warmenu.subsurface(
            (warmenu_left, 0, warmenu_width, bmp_warmenu.get_height())
        )
        ...
        bmp_province_name = Helper.DrawText(
            province_name, (0xFF, 0xFF, 0xFF), palette_no=0, scaled=True
        )
        bmp.blit(bmp_province_name, (warmenu_left + 16, 20))

        date = "{}{}  {}{}".format(
            Data.BUF[0x46] + 1, Helper.GetBuiltinText(0x5849), "1", Helper.GetBuiltinText(0x584E)
        )
        ...
        if wind_no == 0:
            bmp_wind = Helper.DrawText(
                Helper.GetBuiltinText(0x9BDF), (0xFF, 0xFF, 0xFF), palette_no=0, scaled=True
            )
            bmp.blit(bmp_wind, (warmenu_left + 150 + bmp_date.get_width() + 30, 20))
        else:
            bmp_wind = Helper.DrawText(
                Helper.GetBuiltinText(0x9BD1), (0xFF, 0xFF, 0xFF), palette_no=0, scaled=True
            )
            bmp.blit(bmp_wind, (warmenu_left + 150 + bmp_date.get_width() + 30, 20))
            ...
            bmp.blit(
                bmp_wind2,
                (warmenu_left + 150 + bmp_date.get_width() + bmp_wind.get_width() + 30 + 20, 20),
            )

        attack_ruler = Ruler.FromNo(Province.FromSequence(province_no).RulerNo).RulerSelf.GetName()
        attacked_ruler = Ruler.FromNo(
            Province.FromSequence(attack_province_no).RulerNo
        ).RulerSelf.GetName()
        ...
        attacked_officer = Officer.FromOffset(
            Province.FromSequence(attack_province_no).GovernorOffset
        )
        ...
        bmp_weather_sub = pygame.transform.scale(
            bmp_weather_sub,
            (
                bmp_weather_sub.get_width() * Helper.Scale,
                bmp_weather_sub.get_height() * Helper.Scale,
            ),
        )
        ...
        bmp_attack_soldier2 = Helper.DrawText(
            Helper.GetBuiltinText(0x9C45, 0x9C4A), palette_no=1, scaled=True
        )
        ...
        bmp_attack_officers2 = Helper.DrawText(
            Helper.GetBuiltinText(0x9C6D, 0x9C72), palette_no=1, scaled=True
        )
        ...
        bmp_attacked_food = Helper.DrawText(
            str(Province.FromSequence(attack_province_no).Food), scaled=True
        )
        ...
        bmp_attacked_gold = Helper.DrawText(
            str(Province.FromSequence(attack_province_no).Gold), scaled=True
        )
        ...
        pygame.draw.rect(
            bmp, (255, 255, 255), (warmenu_left + 15, 518, bmp.get_width() - warmenu_left - 30, 270)
        )
        commands = Helper.GetBuiltinText(0x9CC4).replace("_", "  ").split("  ")
        for row in range(0, 2):
            for col in range(0, 3):
                bmp_cmd = Helper.DrawText(
                    commands[3 * row + col], (0xFF, 0xFF, 0xFF), palette_no=4, scaled=True
                )
                bmp.blit(bmp_cmd, (warmenu_left + 30 + 130 * col, 530 + 60 * row))

        bmp_attack_ruler = Helper.DrawText(
            attack_ruler + ",", (0xFF, 0xFF, 0xFF), palette_no=0, scaled=True
        )
        ...
        Helper.GetInput(
            Helper.GetBuiltinText(0x9D00, 0x9D01)
            + attack_officer.GetName()
            + Helper.GetBuiltinText(0x9D05, 0x9D09)
            + " (1-6)? ",
            int((warmenu_left + 30) / 2),
            int(720 / 2),
    ...
    def draw_wind(self, wind_no):
        ...
        pygame.draw.polygon(
            wind_angles,
            (0x0, 0x0, 0x0),
            [(0, 0), (10, 0), (12, 2), (4, 2), (16, 14), (14, 16), (2, 2), (2, 8), (0, 6)],
        )
        pygame.draw.polygon(
            wind_direct,
            (0x0, 0x0, 0x0),
            [(8, 0), (0, 6), (0, 8), (6, 6), (6, 16), (10, 16), (10, 6), (16, 8), (16, 6)],
        )
        ...
        return pygame.transform.smoothscale(
            img, (img.get_width() * Helper.Scale, img.get_height() * 2 * Helper.Scale)
        )
