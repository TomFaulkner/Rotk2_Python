...
class Helper(object):
    ...
    def Init(scale, current_province_no):
        ...
        Helper.Palettes = [
            ...
            pygame.color.Color(0x55, 0xFF, 0x55),
            pygame.color.Color(0xFF, 0x55, 0x55),
            pygame.color.Color(0xFF, 0xFF, 0x55),
            pygame.color.Color(0x55, 0x55, 0xFF),
            pygame.color.Color(0x55, 0xFF, 0xFF),
            pygame.color.Color(0xFF, 0x55, 0xFF),
            pygame.color.Color(0xFF, 0xFF, 0xFF),
        ]

        Helper.RulerPalettes = [
            pygame.color.Color(0xAA, 0xAA, 0xAA),
            pygame.color.Color(0xFF, 0xFF, 0x55),
            pygame.color.Color(0xAA, 0xAA, 0x55),
            pygame.color.Color(0xAA, 0x55, 0xFF),
            pygame.color.Color(0xFF, 0x55, 0xFF),
            pygame.color.Color(0xAA, 0x55, 0xFF),
            pygame.color.Color(0x55, 0xFF, 0x80),
            pygame.color.Color(0x55, 0xFF, 0xFF),
            pygame.color.Color(0xAA, 0xFF, 0xFF),
            pygame.color.Color(0x55, 0x55, 0xFF),
            pygame.color.Color(0xFF, 0x55, 0xFF),
            pygame.color.Color(0xAA, 0x00, 0x55),
            pygame.color.Color(0xFF, 0xAA, 0x55),
            pygame.color.Color(0xAA, 0xAA, 0x55),
            pygame.color.Color(0x00, 0x80, 0x00),
    ...
    def get_ruler_rgb(b, palette_no):
        ...
        if b == 0:
            ...
            return (
                Helper.RulerPalettes[palette_no].r,
                Helper.RulerPalettes[palette_no].g,
                Helper.RulerPalettes[palette_no].b,
            )
    ...
    def GetInput(
        ...
        row=0,
        width=300,
        cursor_user_prompt_location=False,
        palette_no=7,
        required_number=True,
        required_number_min=-1,
        required_number_max=-1,
        ...
        if len(next_prompt.strip()) > 0:
            bmp = Helper.DrawText(
                next_prompt, back_color=back_color, palette_no=palette_no, scaled=True
            )
            ...
            pygame.draw.rect(
                ...
                (
                    x * Helper.Scale,
                    (y + row_height * (row + 1)) * Helper.Scale,
                    width * Helper.Scale,
                    bmp.get_height(),
                ),
            )
    ...
    def DrawText(text, back_color=(0, 0, 0), palette_no=7, scaled=False):
        ...
        return pygame.transform.smoothscale(
            bmp, (bmp.get_width() * scale_value, bmp.get_height() * scale_value * 2)
        )

    @staticmethod
    def GetBuiltinTextOne(offset):
        """Get text for a single character/byte at the given offset."""
        if offset >= len(Data.DSBUF):
            return ""

        b = Data.DSBUF[offset]

        # Build Chinese text representation
        if b == 0:
            chinese_text = ""
        elif b == 0x01:
            chinese_text = "@"
        ...
        if translations:
            return translations.get_text(offset, chinese_text)
    ...
    def GetBuiltinText(offset, end=99999999):
        buf = []
        start_offset = offset  # Remember the starting offset for translation lookup

        while True:
            if offset > end:
                chinese_text = "".join(buf)
                # Try to get English translation
                if translations:
                    return translations.get_text(start_offset, chinese_text)
                return chinese_text
            if offset >= len(Data.DSBUF):
                break
        ...
        # Final return if we exit the loop without hitting a null terminator
        chinese_text = "".join(buf)
        if translations:
            return translations.get_text(start_offset, chinese_text)
        return chinese_text
    ...
    def GetOfficerList(
        ...
        if show_flag != ShowOfficerFlag.Empty:
            mappings = {
                ShowOfficerFlag.Int: Helper.GetBuiltinText(0x3FD0),
        ...
        for i in range(8 * page, 8 * (page + 1)):
            ...
            if (
                can_action is True
                and officer_list[i].CanAction() is False
                and enemy_province is False
            ):
                palette_no = 2
            name = Helper.DrawText(officer_list[i].GetName(), palette_no=palette_no)
            mappings = {
                ShowOfficerFlag.Empty: 0,
                ShowOfficerFlag.Int: officer_list[i].Int,
                ShowOfficerFlag.War: officer_list[i].War,
                ShowOfficerFlag.Chm: officer_list[i].Chm,
    ...
    def CursorShining(prompt, cursor_x, cursor_y, back_color=(0, 0, 0), palette_no=7):
        ...
        pygame.draw.rect(
            Helper.Screen,
            back_color,
            (cursor_x, cursor_y, bmp.get_width(), bmp.get_height()),
        )

        Helper.Screen.blit(bmp, (cursor_x, cursor_y))
        pygame.draw.rect(
            Helper.Screen,
            cursor_background[Helper.cur_flag],
            (cursor_x + bmp.get_width(), cursor_y, 8 * Helper.Scale, 32 * Helper.Scale),
            0,
        )
    ...
    def GetMap(color_map=True):
        list = [
            Data.GrpdataMappings["MapTop"],
            Data.GrpdataMappings["MapBottom"],
        ...
        return pygame.transform.scale(
            map, (map.get_width() * Helper.Scale, map.get_height() * Helper.Scale)
        )
    ...
    def GetMapLines(map_buf):
        ...
        for p in range(0, 41):
            p1 = Data.GetWordFromOffset(
                Data.BUF, Data.PROVINCE_START + Data.PROVINCE_SIZE * p + 0x1C
            )
            p2 = Data.GetWordFromOffset(
                Data.BUF, Data.PROVINCE_START + Data.PROVINCE_SIZE * p + 0x1E
            )
    ...
    def GetProvinceName(prov_no, without_no=False):
        ...
        if without_no is True:
            ...
            prov_name = (
                ...
                + Helper.GetBuiltinText(0x6309, 0x630A)
                + "-"
                + str(prov_no)
            )
    ...
    def GetGovernorInformation(prov_no):
        ...
        if ruler_no == 255:
            empty_bmp = Helper.DrawText(
                Helper.GetBuiltinText(0x6310), back_color=(255, 255, 255), palette_no=0
            )
        ...
        ruler_bmp = Helper.DrawText(
            Helper.GetBuiltinText(0x6400), back_color=(255, 255, 255), palette_no=2
        )
        bmp.blit(ruler_bmp, (60, 20))

        officer_bmp = Helper.DrawText(
            Helper.GetBuiltinText(0x6405), back_color=(255, 255, 255), palette_no=4
        )
        ...
        if advisor_offset > 0:
            ...
            for officer in offlist:
                if officer.Offset == advisor_offset:
                    ...
                    break
            if exist_advisor:
                assistant_bmp = Helper.DrawText(
                    Helper.GetBuiltinText(0x640A),
                ...
                advisor_offset_bmp = Helper.DrawText(
                    advisor_name_data, back_color=(255, 255, 255), palette_no=0
                )
    ...
    def DrawDate():
        ...
        info = "{0}{3} {1}{4} {2}".format(
            year,
            month,
            season,
            Helper.GetBuiltinTextOne(0x3DD2),
    ...
    def GetProvinceInformation(prov_no):
        ...
        polutions_num = (Data.BUF[(province_offset + 0x0F)] << 8) + (
            Data.BUF[(province_offset + 0x0E)] << 0
        )
        ...
        food = Helper.DrawText(
            Helper.GetBuiltinText(0x6263, 0x6266),
            palette_no=3,
        )
        food_num = (
            (Data.BUF[(province_offset + 0x0D)] << 24)
            + (Data.BUF[(province_offset + 0x0C)] << 16)
            + (Data.BUF[(province_offset + 0x0B)] << 8)
            + (Data.BUF[(province_offset + 0x0A)] << 0)
        )
    ...
    def SelectOfficer(
        ...
        while True:
            if clear_area is True:
                Helper.ClearInputArea()
            bmp, have_more_pages, officer_status = Helper.GetOfficerList(
                officer_list,
                show_flag,
                page,
                multi_select,
            ...
            if only_one_page:
                yn = Helper.GetInput(
                    prompt + " (1-{0})? ".format(len(officer_list)),
                    required_number_min=1,
                    required_number_max=len(officer_list),
                    row=row,
                )
            else:
                yn = Helper.GetInput(
                    prompt + " (0-{0})? ".format(len(officer_list)),
                    next_prompt=Helper.GetBuiltinText(0x6393),
    ...
    def DisplayProvinces(ruler_no, page):
        ...
        taishou = Helper.DrawText(
            Helper.GetBuiltinText(0x6405), back_color=(255, 255, 255), palette_no=0
        )
        huangjin = Helper.DrawText(
            Helper.GetBuiltinText(0x7C37), back_color=(255, 255, 255), palette_no=1
        )
        ...
        shibing = Helper.DrawText(
            Helper.GetBuiltinText(0x6251, 0x6254),
            back_color=(255, 255, 255),
            palette_no=2,
        )
        zhongcheng = Helper.DrawText(
            Helper.GetBuiltinText(0x623F, 0x6242),
            back_color=(255, 255, 255),
            palette_no=1,
            scaled=False,
        )
        jiangling = Helper.DrawText(
            Helper.GetBuiltinText(0x6528, 0x652C),
            back_color=(255, 255, 255),
            palette_no=6,
            scaled=False,
        )
        ...
        for province in province_list:
            ...
            body.blit(
                governor_name,
                (
                    x_list[1] + (x_list[2] - x_list[1] - governor_name.get_width()) / 2,
                    height + 2,
                ),
            )
            huangjin = Helper.DrawText(
    ...
    def RenderColorText(text_list, x, y):
        ...
        for ct in text_list:
            img = Helper.DrawText(
                ct.text,
                back_color=Helper.Palettes[int(ct.back_color)],
                palette_no=int(ct.fore_color),
            )
            img = pygame.transform.scale(
                img, (img.get_width() * Helper.Scale, img.get_height() * Helper.Scale)
            )
    ...
    def GetCurrentRulerOfficerOffset():
        return (
            Data.BUF[Data.CURRENT_RULER_OFFICER_OFFSET + 1] * 256
            + Data.BUF[Data.CURRENT_RULER_OFFICER_OFFSET + 0]
        )
