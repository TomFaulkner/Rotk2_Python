...
class Command11(object):
    ...
    def Start(self, province_no):
        commands = [
            Helper.GetBuiltinText(0x7C37),
            Helper.GetBuiltinText(0x7C3C),
            Helper.GetBuiltinText(0x7C41),
        ]
        ...
        Helper.ShowCommandsInInputArea(
            commands, 3, palette_no=3, width=70, commands_color=commands_color
        )
        award = Helper.GetInput(
            Helper.GetBuiltinText(0x7C4A) + "(1-3)? ",
            row=1,
        ...
        if award == 1:
            ...
            if whos > 0:
                ...
                gold = Helper.GetInput(
                    Helper.GetBuiltinText(0x7C09) + "(1-100)? ",
                    required_number_min=1,
                    required_number_max=100,
                )
                if gold > 0:
            ...
            if whos > 0:
                ...
                if result == -1:
                    ...
                    Helper.ShowDelayedText(
                        Helper.GetBuiltinText(0x7BA6).replace("%s", officer.GetName()), palette_no=3
                    )
    ...
    def ShowRewardResult(self, officer, result):
        if result == -1:
            ...
            info = (
                Helper.GetBuiltinText(0x7BC9)
                .replace("%s", officer.GetName())
                .replace("%d", str(result))
            )
            palette_no = 3
