...
class Command9_10_12(object):
    ...
    def Start(self, province_no):
        ...
        if who > 0:
            ...
            really_cost = Helper.GetInput(
                self.resources + "(1-{0})? ".format(max_cost),
                row=1,
                cursor_user_prompt_location=True,
                required_number_min=1,
                required_number_max=max_cost,
            )
            if really_cost > 0:
                province.__setattr__(self.cost_field, resources - int(really_cost))

                if self.affected_field != "Loyalty":
                    diff = self.game_development_land_flood_calc(
                        province_no, who, really_cost, self.affected_field
                    )
                else:
...
class Command9(Command9_10_12):
    def __init__(self):
        super().__init__(
            Helper.GetBuiltinText(0x7A41),
            Helper.GetBuiltinText(0x7A4F),
            Helper.GetBuiltinText(0x7A6C, 0x7A79),
            ShowOfficerFlag.Int,
            "Gold",
            "Land",
            100,
...
class Command10(Command9_10_12):
    def __init__(self):
        super().__init__(
            Helper.GetBuiltinText(0x7AD0),
            Helper.GetBuiltinText(0x7A4F),
            Helper.GetBuiltinText(0x7AFB, 0x7B06),
            ShowOfficerFlag.Int,
            "Gold",
            "Flood",
            100,
...
class Command12(Command9_10_12):
    def __init__(self):
        super().__init__(
            Helper.GetBuiltinText(0x7CF5),
            Helper.GetBuiltinText(0x7CFF),
            Helper.GetBuiltinText(0x7D14, 0x7D20),
            ShowOfficerFlag.Chm,
            "Food",
            "Loyalty",
            10000,
