...
class Command1(object):
    ...
    def Start(self, province_no):
        ...
        while True:
            ...
            where = Helper.GetInput(
                Helper.GetBuiltinText(0x6998) + "(1-41)? ",
        ...
        whos = Helper.SelectOfficer(
            province_no, Helper.GetBuiltinText(0x695C), ShowOfficerFlag.Soldiers, True
        )
        if len(whos) == 0:
            return

        gold = Helper.GetInput(
            Helper.GetBuiltinText(0x6965) + " (0-{0})?".format(province.Gold),
            row=1,
            required_number_min=0,
            required_number_max=province.Gold,
        )
        if gold < 0:
            return

        food = Helper.GetInput(
            Helper.GetBuiltinText(0x6974, 0x697F) + " (0-{0})?".format(province.Food),
            required_number_min=0,
            required_number_max=province.Food,
            row=2,
        )
        giveup = False
        ...
        if ruler_moved:
            ...
            if giveup:
                ...
                prompt = prompt.replace(
                    "%s", Ruler.GetNameFromProvinceSequence(province_no)
            ...
            prompt = Helper.GetBuiltinText(0x3F75)
            prompt = prompt.replace("%s", Ruler.GetNameFromProvinceSequence(province_no)).replace(
                "%2d", str(where)
            )
            prompt += "(1-{0})?".format(len(whos))
            ...
            Province.SetGovernor(
                where, Province.FromSequence(where).GetOfficerList()[governor - 1].Offset
            )
            ...
            if governor_moved:
                if giveup:
                    ...
                    governor = Helper.SelectOfficer(province_no, prompt, ShowOfficerFlag.Loyalty)
                    Province.SetGovernor(
                        province_no, province.GetOfficerList()[governor - 1].Offset
                    )
