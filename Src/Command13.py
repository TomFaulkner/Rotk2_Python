...
class Command13(object):
    ...
    def Start(self, province_no):
        ...
        officer_no = Helper.SelectOfficer(
            province_no, Helper.GetBuiltinText(0x7E63), ShowOfficerFlag.Empty
        )
        if officer_no == 0:
            return
    ...
    def SellFood(self, officer_no):
        ...
        food_sold = Helper.GetInput(
            Helper.GetBuiltinText(0x7DE6, 0x7DEF) + "({0}-{1})? ".format(food_min, food_max),
            required_number_min=food_min,
            required_number_max=food_max,
        )
        ...
        if food_sold > 0:
            ...
            Helper.ShowDelayedText(
                Helper.GetBuiltinText(0x7DAD, 0x7DC2)
                .replace("%u", str(self.province.Gold))
                .replace("%lu", str(self.province.Food))
            )
    ...
    def BuyFood(self, officer_no):
        ...
        food_buy = Helper.GetInput(
            Helper.GetBuiltinText(0x7DD1).replace("%lu", str(food_max)),
            required_number_min=1,
            required_number_max=food_max,
        )
    ...
    def BuyHorse(self, officer_no):
        ...
        num = Helper.GetInput(
            Helper.GetBuiltinText(0x7D7A) + "(1-{0})? ".format(horses_can_buy),
            required_number_min=1,
        ...
        if num > 0:
            ...
            self.province.Flush()
            Helper.ShowDelayedText(
                Helper.GetBuiltinText(0x7D8C, 0x7D9A).replace(
                    "%d", str(Data.BUF[self.province.Offset + 0x19])
                )
            )
    ...
    def BuyWeapons(self, officer_no):
        ...
        weapons_buy = Helper.GetInput(
            Helper.GetBuiltinText(0x7D58) + "(1-{0})? ".format(weapons_max),
            required_number_min=1,
            required_number_max=weapons_max,
        )
        if weapons_buy > 0:
            ...
            Helper.ShowDelayedText(
                Helper.GetBuiltinText(0x7D6B)
                .replace("%s", officer.GetName())
                .replace("%d", str(officer.Weapons))
            )

    def MerchantCheckExist(
        self,
    ):
