...
class Command2(object):
    ...
    def Start(self, province_no):
        ...
        whos = Helper.SelectOfficer(
            province_no, Helper.GetBuiltinText(0x6A23), ShowOfficerFlag.Soldiers
        )
        if whos == -1:
            return

        gold = Helper.GetInput(
            Helper.GetBuiltinText(0x69EF) + " (0-{0})?".format(province.Gold), row=1
        )
        if len(gold) == 0:
            return

        food = Helper.GetInput(
            Helper.GetBuiltinText(0x69FC).replace("%lu%", str(province.Food)), row=2
        )
        if len(food) == 0:
            return
