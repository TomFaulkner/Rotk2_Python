...
class DrawCGA(object):
                    ...
                    if (self.r.AL & 0x80) != 0x80:
                        ...
                        if is_odd == 1:
                            ...
                            while disp_index_offset > 0:
                                self.display_buf[self.disp_index] = (
                                    int(new_pixel / 16)
                                    + int(self.display_buf[self.disp_index] / 16) * 16
                                )
                                self.disp_index += 1
                                disp_index_offset -= 1
                                ...
                                if disp_index_offset > 0:
                                    self.display_buf[self.disp_index] = (
                                        int(new_pixel / 16) * 16
                                        + self.display_buf[self.disp_index] % 16
                                    )
                                    disp_index_offset -= 1
                        else:
                            while disp_index_offset > 0:
    ...
    def Draw_Pixel_4(self):
        self.display_buf[self.disp_index] = (
            int(self.display_buf[self.disp_index] / 16) + int(self.display_buf[self.r.SI] / 16) * 16
        )
        self.r.CX -= 1
    ...
    def Draw_Pixel_2(self):
        self.display_buf[self.disp_index] = (
            int(self.display_buf[self.disp_index] / 16) * 16 + self.display_buf[self.r.SI] % 16
        )
        self.r.SI += 1
        self.disp_index += 1
        self.r.CX -= 1
    ...
    def Draw_Pixel_3(self):
        self.display_buf[self.disp_index] = (
            int((self.display_buf[self.r.SI] * 16) % 256 / 16 * 16)
            + self.display_buf[self.disp_index] % 16
        )
        self.r.SI += 1
        self.r.CX -= 1
    ...
    def Draw_Pixel_6(self):
        self.display_buf[self.disp_index] = (
            int(self.r.DL / 16) * 16 + self.display_buf[self.disp_index] % 16
        )
        self.r.CX -= 1

    def Draw_Pixel_1(self):
        self.display_buf[self.disp_index] = (
            int(self.display_buf[self.r.SI] / 16) + int(self.display_buf[self.disp_index] / 16) * 16
        )
    ...
    def Draw_Pixel_5(self):
        self.display_buf[self.disp_index] = (
            int(self.r.DL / 16) + int(self.display_buf[self.disp_index] / 16) * 16
        )
        self.disp_index += 1
        self.r.CX -= 1
