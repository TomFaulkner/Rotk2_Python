...
def extract_all_provinces(image_path: str):
    ...
    for prov in provinces:
        ...
        cx, cy = prov["center"]
        cv2.putText(
            debug_img, str(pid), (cx - 10, cy + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2
        )
    ...
    print(
        f"  Points per province: min={min(point_counts)}, max={max(point_counts)}, avg={sum(point_counts) / len(point_counts):.1f}"
    )
